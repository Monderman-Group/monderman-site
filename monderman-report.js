...
(function () {
  ...
  function renderThreeBenefitFlow(s,level) {
    ...
    const sources=[],colors={spendingReduction:'#0C6E78',spendingAvoidance:'#C9A227',staffCapacity:'#4E8991'};
    for(const [key,rows,field] of [['spendingReduction',s.spendingItems.filter(x=>x.category==='spendingReduction'),'amount'],['spendingAvoidance',s.spendingItems.filter(x=>x.category==='spendingAvoidance'),'amount'],['staffCapacity',s.activities,'capacityValue']]){
      ...
      for(const row of visible)if(row[field][level]>0)sources.push({label:row.label,key,type:THREE_BENEFIT_LABELS[key],amount:row[field][level]});
      if(rest.length){const amount=rest.reduce((n,row)=>n+row[field][level],0);if(amount>0)sources.push({label:'Other '+({staffCapacity:'activities',spendingReduction:'current expenses',spendingAvoidance:'planned expenses'}[key])+' ('+rest.length+')',key,type:THREE_BENEFIT_LABELS[key],amount});}
    }
    if(!sources.length||THREE_BENEFIT_KEYS.some(key=>s.benefits[key].amount[level]>0&&!sources.some(item=>item.key===key)))return '<p class="mr-benefit-chart-note">'+(THREE_BENEFIT_KEYS.some(key=>s.benefits[key].amount[level]>0)?'Some source values are too small to draw after currency rounding. See the exact saved totals in the planning table.':'The reviewed benefits are zero in this case. Costs and net values remain in the planning table; there are no benefit flows to draw.')+'</p>';
    const dest=THREE_BENEFIT_KEYS.map(key=>({key,label:THREE_BENEFIT_LABELS[key],amount:s.benefits[key].amount[level]})).filter(item=>item.amount>0);
    const rows=sources.length,height=rows*88+32,maximum=Math.max(...sources.map(x=>x.amount)),scale=52/maximum;
    // Each source joins its actual benefit category. This is a gross-benefit
    // diagram, not an allocation of retained staff time to cash expenditure.
    for(const target of dest){
      const members=sources.map((item,index)=>({...item,index})).filter(item=>item.key===target.key);
      const drawnAmount=members.reduce((sum,item)=>sum+item.amount,0);
      target.center=members.reduce((sum,item)=>sum+(48+item.index*88)*item.amount,0)/drawnAmount;
      target.offset=target.center-drawnAmount*scale/2;
    }
    let paths='',nodes='';
    const label=(item,right,center)=>'<div class="mr-benefit-flow-node'+(right?' is-right':'')+'" data-flow-category="'+item.key+'" style="--benefit-node-center:'+(center/height*100)+'%;--benefit-color:'+colors[item.key]+'" title="'+esc(item.label)+'"><span>'+esc(item.label)+'</span><strong data-saved-value="'+item.amount+'">'+esc(threeBenefitHeadlineMoney(item.amount))+'</strong></div>';
    sources.forEach((item,index)=>{
      const target=dest.find(node=>node.key===item.key),width=item.amount*scale,start=48+index*88-width/2,end=target.offset;
      target.offset+=width;
      paths+='<path d="M190 '+start+' C330 '+start+',430 '+end+',570 '+end+' L570 '+(end+width)+' C430 '+(end+width)+',330 '+(start+width)+',190 '+(start+width)+' Z" fill="'+colors[item.key]+'" data-flow-category="'+item.key+'" data-three-benefit-flow-value="'+item.amount+'"><title>'+esc(item.label+' to '+item.type+': '+threeBenefitMoney(item.amount))+'</title></path>';
      nodes+=label(item,false,48+index*88);
    });
    for(const target of dest)nodes+=label(target,true,target.center);
    ...
    const style='<style>.mr-benefit-chart[data-flow-version="20260920.1"]{margin:24px 0}.mr-benefit-chart[data-flow-version]>.mr-benefit-scroll{overflow-x:auto;overflow-y:hidden;margin:12px 0 16px}.mr-benefit-chart[data-flow-version] .mr-benefit-flow{min-width:760px;height:calc(var(--benefit-rows)*88px + 32px)!important}.mr-benefit-chart[data-flow-version] .mr-benefit-flow-node{top:var(--benefit-node-center)!important;transform:translateY(-50%);width:24%;min-height:0;padding:6px 10px;font-size:13px;line-height:1.35;border-color:var(--benefit-color)}.mr-benefit-chart[data-flow-version] .mr-benefit-flow-node span{-webkit-line-clamp:3}.mr-benefit-chart[data-flow-version] .mr-benefit-flow-node strong{font-size:15px}.mr-benefit-flow-heading{display:flex;justify-content:space-between;gap:20px;font-size:.8rem;font-weight:700;color:#53676E;margin-top:18px}.mr-benefit-flow-heading span:last-child{text-align:right}.mr-benefit-flow-hint{font-size:12px;color:#53676E}@media screen and (min-width:1000px){.mr-benefit-flow-hint{display:none}}@media print{.mr-benefit-chart[data-flow-version] .mr-benefit-flow{min-width:0;height:calc(var(--benefit-rows)*42px + 16px)!important}.mr-benefit-chart[data-flow-version]>.mr-benefit-scroll{overflow:visible}.mr-benefit-chart[data-flow-version] .mr-benefit-flow-node{font-size:8px;line-height:1.2;padding:3px 5px}.mr-benefit-chart[data-flow-version] .mr-benefit-flow-node strong{font-size:9px}.mr-benefit-flow-heading{font-size:9px;margin-top:8px}.mr-benefit-flow-hint{display:none}}</style>';
    ...
    return '<figure class="mr-benefit-chart" data-flow-version="20260920.1">'+style+'<h4>'+humanize(level)+' case: how the value adds up</h4><p class="mr-benefit-chart-note">Activities and expenses flow into the three benefit totals. These are gross benefits, before costs. Retained staff capacity is time available for other work, not cash savings. Costs and net value are shown separately.</p><div class="mr-benefit-flow-heading"><span>Where the value comes from</span><span>Benefit totals</span></div><p class="mr-benefit-flow-hint">Scroll across the diagram to see the full flow. You can also focus the diagram and use the arrow keys.</p><div class="mr-benefit-scroll" role="region" tabindex="0" onkeydown="'+esc(keyboard)+'" aria-label="'+humanize(level)+' planning-value Sankey"><div class="mr-benefit-flow" style="--benefit-rows:'+rows+'"><svg viewBox="0 0 760 '+height+'" preserveAspectRatio="none" role="img" aria-label="'+humanize(level)+' operational benefit flows"><desc>'+esc(dest.map(item=>item.label+' '+threeBenefitMoney(item.amount)).join('; '))+'. Costs and net planning value are shown separately.</desc>'+paths+'</svg><div class="mr-benefit-flow-labels">'+nodes+'</div></div></div><div class="mr-benefit-compact-flow">'+compact('Source activities and expenses',sources)+compact('Benefit totals',dest)+'</div><figcaption class="mr-benefit-chart-note">Directional estimates from recorded operational inputs and stated assumptions. Ribbon width represents dollar value, not hours. Grouping is consistent across all three cases; every source item is listed in full below.</figcaption></figure>';
  }
