...
# September 24 approved transition correction on page 10; the cover and other
# 12 pages were rendered and compared pixel-identically against the prior PDF.
elif hashlib.sha256(built_pdf.read_bytes()).hexdigest()!='359f7d2121514eb3df8e2eed589e79cb89031b7c2cabdb613b8a0e93471fa5a4':
...
# September 24: exact plain-English equivalents; all access and secret boundaries remain required.
for token in ['Monderman Workspace is available by invitation.','Direct Diagnostics require a signed-in member with active Workspace access.','New organizations need a valid invitation to activate their evaluation.','Directed campaign links authorize only the assigned Diagnostic and do not make the recipient a Workspace member.','Monderman uses database row-level security and server-side authorization to restrict Workspace data access','Database policies and permissions limit users to the operations their role allows.','Tables reserved for server use can have row-level security enabled with no customer policies, so browser roles cannot read or write those rows.','public publishable key','Privileged database, payment, AI and email credentials stay on the server and are not shipped to the browser.','Anthropic\'s commercial API','durable redemption record','Deleting a Workspace does not create another evaluation','does not currently claim SOC 2','four-hour cutoff','durable Supabase snapshot','plan, usage, billing and stored Diagnostic result fields remain server-managed','request-size and rate limits','Controlled release checks currently cover current Chrome/Chromium and automated WebKit rendering','Native Safari and browser-managed print dialogs remain best-effort']:
