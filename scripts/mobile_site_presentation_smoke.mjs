...
const immutableLegalPages = new Set(pages.filter((name) => /^(?:terms|privacy)-\d{4}-\d{2}-\d{2}-(?:beta|optional-measurement-v1|ai-evidence-v1)\.html$/.test(name)));

// September 11 adds an immutable Privacy edition while Terms remain September 9.
// Preserve every previous legal edition in the full viewport sweep.
assert.equal(pages.length, 77, 'rendered root-page inventory changed unexpectedly');
assert.equal(canonicalPages.length, 59, 'canonical header + footer inventory changed unexpectedly');
assert.equal(footerPages.length, 63, 'footer inventory changed unexpectedly');
...
assert.equal(immutableLegalPages.size, 13, 'immutable legal-page inventory changed unexpectedly');
