...
export function assertHomepageSampleDiscovery(root=path.resolve(import.meta.dirname,'..')){
  ...
  assert.equal(homepagePreviewBeforeDiscoverability(template),prior('scripts/templates/home-workspace-preview.html'),'The template changes only the permanent sample link');
  ...
  assert.match(preview,/<\/section>\s*<\/div>\s*<div class="hwd-footer">[\s\S]*class="hwd-sample-link" href="sample-report\.html"/,'The direct sample link is outside every journey panel');
  ...
  assert.equal(preview,generated.hero,'Displayed preview matches the unchanged sample data and current template');
  ...
  for(const file of ['homepage-workspace-demo.js','scripts/refresh_public_sample_previews.mjs','enterprise-site.css','assets/brand/brand-foundations-v2.css','monderman-shell.css','dv-result-dialog.css','sample-data/production-diagnostic-samples.json']){
    assert.equal(read(file),prior(file),file+': journeys, palette roles, semantic states and source data are unchanged');
  }
  ...
  assert.match(shared,/--mdm-gold:\s*#C9A227/);
  ...
  return {passed:true,sourceDataUnchanged:true,journeyRuntimeUnchanged:true,semanticColorsUnchanged:true,generatedPreviewMatches:true,browserVerification:false,publicationApprovalClaimed:false};
}
