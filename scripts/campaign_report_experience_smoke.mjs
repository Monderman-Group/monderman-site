...
assert.deepEqual(errors,[]);await browser.close();const receipt={ok:true,mode:privateMode?'private-engine-mock-transport':'public-ui-mock',sourceCommit,engineReadinessExecuted:privateMode,rawScorePersistenceVerified:false,widths:[1440,834,390,320],screenshots:12,applyCalls,productionCalls:0,checks:'Displayed states, preview invalidation, explicit apply, single request on doubletap, unchanged mock score, forms and horizontal overflow. No live API, database persistence or end-to-end integration claim.'};fs.writeFileSync(path.join(out,'checks.json'),JSON.stringify(receipt,null,2));console.log(JSON.stringify(receipt));
