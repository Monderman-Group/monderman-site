...
function depthCard(place) {
  ...
  if(s!==null&&s!==undefined){
   if(s.version==='operational-planning-scenario-20260919.2'){
    ...
    opportunity='<div class="md-opportunity"><span>'+(c.status==='estimated'?whole(c.measuredPeople)+' measured people · ':'')+whole(s.inputs.horizonMonths)+' months · Central case</span>'+capacityHeadline+'<p>Retained staff capacity value, not cash savings. '+statusLabel(b.staffCapacity)+'.</p><dl class="md-scenario-cases" data-three-benefit-cases>'+['low','central','high'].map(k=>'<div><dt>'+({low:'Low',central:'Central',high:'High'}[k])+'</dt><dd>'+amount(b.staffCapacity.amount,k,rounded)+'</dd></div>').join('')+'</dl><p>Named input cases, not ordered bounds. Full assumptions and exact values in the report.</p></div>';
    economics='<div class="md-economics"><div><strong data-promo-spending-reduction>'+central(b.spendingReduction.amount)+'</strong><span>Cash spending reduced · central case. '+statusLabel(b.spendingReduction)+'.</span></div><div><strong data-promo-spending-avoidance>'+central(b.spendingAvoidance.amount)+'</strong><span>Future spending avoided · central case. '+statusLabel(b.spendingAvoidance)+'.</span></div><div><strong data-promo-net-cash>'+central(t.netCashEffect)+'</strong><span>Net spending effect versus current and planned baselines, after cash costs · central case</span></div><div><strong data-promo-total-cost>'+money(t.totalImplementationAndSubscriptionCost.central)+'</strong><span>Implementation and subscription cost, including internal staff time · central case</span></div></div>';
    basis=benefitCoverage(s)+' Spending uses separate current and planned baselines. Capacity excludes hours assigned to spending benefits. These are illustrative assumptions, not measured cash savings.';
   }else{
