...
try{
  for(const kind of ['uncited-consecutive','referenced-final-check','over-page-long-action']){
    const value=structuredClone(run),recommendations=structuredClone(actions);
    ...
    value.ai_report={status:'complete',report:{...value.ai_report.report,composition:{reviewed_version:'report-reviewed-capabilities-20260909.1'},interpretation:{summary:'Synthetic layout test.',observations:[],hypotheses:[],recommendations,limitations:[]},sources:[{id:'test-reference',title:'Synthetic reference',publisher:'Synthetic publisher',url:'https://www.monderman.com/research.html',reviewed:'2026-09-09'}]}};
    ...
    for(const width of [390,768,1440]){
      ...
      const facts=await page.locator('.mr-ai-action').evaluateAll(es=>es.map(e=>({text:e.innerText,header:e.querySelector('h3').innerText,tail:e.querySelector('.mr-ai-definition:last-child').innerText,reference:e.querySelector('dl+p')?.innerText||null,after:getComputedStyle(e.querySelector('dl')).breakAfter,tailAfter:getComputedStyle(e.querySelector('.mr-ai-definition:last-child')).breakAfter,inside:getComputedStyle(e).breakInside})));
      assert.equal(facts.length,2);for(const f of facts){assert.equal(f.inside,'auto');assert.equal(f.after,f.reference?'avoid':'auto');assert.equal(f.tailAfter,f.reference?'avoid':'auto');}
      ...
      else for(const f of facts)assert.ok(texts.some(t=>t.includes(compact(f.text))),`Short whole action split: ${kind}`);
...
console.log(JSON.stringify({passed:receipt.passed,out,pdfs:9,providerCalls:0}));
