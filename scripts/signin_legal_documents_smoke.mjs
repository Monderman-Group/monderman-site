...
const docs = version => ({ok:true, requiresAcceptance:true, termsVersion:'2026-09-09-beta', privacyNoticeVersion:version});
let checks = 0;
...
for(const file of ['signin.html','pattern-trial.html']){
  ...
  eq(actual.length,14,'complete explicit archive inventory');
...
for(const scenario of ['v2','v1','beta','unknown','decline','acceptance_failed']){
  ...
  const ctx=vm.createContext({ui,forwarded:false,nextTarget:'workspace.html',invitationMode:false,API_BASE:'https://mock.invalid',
    ...
    fetch:async(url,options={})=>{calls.push({url,options});const failed=options.method==='POST'&&scenario==='acceptance_failed';return {ok:!failed,json:async()=>options.method==='POST'?{ok:!failed}:docs(version)};}});
  ...
  eq(ui.legalPrivacyLink.href,'privacy-'+version+'.html');eq(ui.legalTermsLink.href,'terms-2026-09-09-beta.html');ok(ui.legalAgree.focused);
  ...
  eq(JSON.parse(calls[1].options.body),{agreed:true,source:'signup',terms_version:'2026-09-09-beta',privacy_notice_version:version});
...
for(const scenario of ['v2','v1','changed','unknown','acceptance_failed']){
  ...
  const ctx=vm.createContext({window:{supabase:{createClient:()=>client}},document:{getElementById:get},
    ...
    setTimeout(){},fetch:async(url,options={})=>{
      ...
      else if(url.includes('/legal/acceptance/status'))body=docs(url.includes('source=trial')&&scenario==='changed'?v2:initial);
  ...
  if(scenario==='changed'){
    eq(posts.length,0,'version transition requires another explicit choice');eq(get('ackStart').checked,false);eq(get('startBtn').disabled,true);eq(get('trialPrivacyLink').href,'privacy-'+v2+'.html');
  }else{
    eq(posts.length,scenario==='acceptance_failed'?1:2);
    eq(JSON.parse(posts[0].options.body),{agreed:true,source:'trial',organization_id:'mock-org',terms_version:'2026-09-09-beta',privacy_notice_version:initial});
