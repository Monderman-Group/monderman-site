...
const keys=['title','measurementStart','measurementEnd','measuredPeople','horizonMonths','activities','implementationCashCost','implementationCapacityCost','subscriptionCost','costBasis','scopeConfirmed','overlapReviewed'];
const activityKeys=['id','label','measuredHours','sourceBasis','sourceReference','loadedHourlyCost','reductionPercent','adoptionPercent','changeBasis','avoidableNonLaborCash','cashBasis'];
...
for(const [name,type]of [['chromium',chromium],['webkit',webkit]]){
  ...
  try{for(const width of [390,834,1440]){
    ...
    async function fillActivity(row,label='MOCK duplicate-entry review'){
      for(const[key,value]of Object.entries({label,measuredHours:'8',sourceReference:'MOCK time-study record set',loadedHourlyCost:'50',changeBasis:'MOCK bounded trial assumptions for this activity only.',cashBasis:'MOCK contract reduction, separate from payroll value.'}))await input(row,key).fill(value);
      ...
      await fillRange(row,'avoidableNonLaborCash',{low:100,central:200,high:300});
    }
    ...
    eq(sent.analysis_mode,'descriptive','Early scenario does not unlock synthesis');eq(Object.keys(scenario),keys,'Exact top-level input only');eq(Object.keys(scenario.activities[0]),activityKeys,'Exact activity input only');
    eq(scenario.measuredPeople,2,'Use entered measured people, not twelve campaign participants');eq(scenario.activities[0].measuredHours,8,'Do not multiply total measured hours');
    ...
    if(calculator){const ctx=current();const result=calculator.calculateFinancialPlanningScenario(scenario,{scope,readiness:ctx.readiness,snapshot:ctx.snapshot,actorId:'actor-mock',createdAt:'2026-09-13T12:00:00Z'});eq(result.totals.netCashEffect,{low:65,central:175,high:285},'Actual server calculator accepts UI input without counting absorbed labor as cash');eq(result.kind,'early_planning_scenario','Actual calculator preserves early status');}
    eq(JSON.stringify(current()),original,'Editing did not change scope, readiness or records');
    await page.evaluate(()=>window.rejectReport=true);const completed=await page.evaluate(()=>window.completed.length);await clickBuild();
    check(await page.locator('[data-ca-message]').textContent().then(text=>text.includes('MOCK server rejected')),'Server rejection displayed safely');eq(await page.evaluate(()=>window.completed.length),completed,'Rejected report not completed or saved');eq(await input(form,'title').inputValue(),'MOCK operational planning','Server error retains editable input');
    ...
    eq(await form.locator('input,select,textarea,button').evaluateAll(nodes=>nodes.filter(el=>el.type!=='checkbox'&&el.getBoundingClientRect().height<44).map(el=>({name:el.name||el.textContent,type:el.type,height:el.getBoundingClientRect().height}))),[],'Inputs and buttons retain usable targets');
