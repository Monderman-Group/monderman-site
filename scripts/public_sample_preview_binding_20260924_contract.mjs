...
for(const [file,entry]of Object.entries(previewBindingDelta)){
  const current=read(file).toString(),prior=sourceBeforePublicSamplePreviewBinding20260924(file,current);
  ...
  eq(depth[0][0],sections[file==='index.html'?'home':'brief'],file+': exact pure generated Depth card');
  if(file==='index.html'){
