...
for(const file of OVERVIEW_SITE_FILES){
  ...
  const currentRejection=CHANGE_WORDING_FILES.includes(file)?/only the exact reviewed current source can be inverted \(change wording\)/:HOMEPAGE_REPORT_QUAD_FILES.includes(file)
  ...
  const priorRejection=CHANGE_WORDING_FILES.includes(file)?currentRejection:HOMEPAGE_REPORT_QUAD_FILES.includes(file)
    ?currentRejection
