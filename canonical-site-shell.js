(() => {
  ...
  const brandMark = '<svg class="monderman-lockup__mark" viewBox="0 0 64 64" aria-hidden="true" focusable="false"><path d="M15 15L23.5 8L32 14L40.5 8L49 15V56L40.5 49L32 55L23.5 49L15 56Z" stroke-width="2.8" stroke-linejoin="round"/><path d="M23.5 8V49M32 14V55M40.5 8V49" stroke-width="2.4" stroke-linecap="round"/></svg>';
  const enhanceBrand = () => {
    document.querySelectorAll(".header .brand, #siteHeader .brand").forEach((brand) => {
      ...
      const textNode = [...brand.childNodes].find((node) => node.nodeType === Node.TEXT_NODE && node.textContent.trim());
      if (textNode) {
    ...
    document.querySelectorAll(".mond-footer .mf-mark-row").forEach((footerBrand) => {
      if (footerBrand.querySelector(".monderman-lockup__mark")) return;
      footerBrand.classList.add("monderman-lockup");
      footerBrand.insertAdjacentHTML("afterbegin", brandMark);
