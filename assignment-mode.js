  ...
  var MondermanAssignment = {
    ...
    privacyNotice: function (cfg) {
      ...
      box.innerHTML =
        ...
        "<p>The quantitative score is calculated by versioned application code from structured answers; AI does not calculate or set it. When AI-assisted reporting is enabled, selected structured answers, computed results and context are sent to Anthropic's commercial API for a separate written interpretation. That interpretation can contain errors and must be reviewed before use. Optional written observations are displayed separately and do not change the score. They are included in AI interpretation only when that separate feature is enabled; the report states when they were not incorporated.</p>" +
