...
The candidate uses non-positioned, bounded inline-block printed options, limits list-item break avoidance to direct section lists, and uses block flow inside synthesis evidence rows. The exact-width corpus test exercised 432 option cards across 144 cases; the largest card including vertical margins was 585.78125px, within the 896px printable height. Explicit computed-style and complete-card text checks guard this print-flow contract. This follows the atomic-box approach described in the [CSS fragmentation specification](https://www.w3.org/TR/css-break-3/).

Correction to the earlier paint-loss diagnosis: Poppler at 90 DPI omitted text in `de1e582` SC page 11 and `dd58860` OS page 10. Re-rendering the identical OS PDF at 72, 144 and 160 DPI displayed the complete heading and all four actions. Independent PDFium at 144 DPI also displayed them, and the earlier SC page 11 displayed all four actions at 160 DPI. Thus those low-resolution images were not proof that Chromium lost the PDF content. The actual `dd58860` Linux artifact ZIP was SHA-256 verified (`70a1c7a8bff9a3f5990196fced7af89f56b4e328e4fa51aa90ce9f9ae25f01fb`); all 74 pages were inspected, with PDFium independently confirming the questioned OS page and six SC/IP option pages. No missing content was found in those independent renders. Earlier artifacts remain evidence, not selected release targets. Ordinary pagination whitespace and the OS page 4/5 time-and-cost heading break remain minor limitations.

CI also exposed a harmless hard-hyphen line wrap (`multi-` followed by `stakeholder` on the next line); PDF text comparison now joins only that line-break pattern, preserving hyphens and ordinary spaces. The PDF gate runs immediately after generation, without removing later release checks. The mobile inventory now includes both September 8 legal archives: 72 root pages, 54 canonical-shell pages and 58 footer pages, with the same 14 functional shell-free pages. Final CI and live publication verification are still required.
