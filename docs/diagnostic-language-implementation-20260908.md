...
The actual Linux CI artifact for `de1e582` subsequently showed the same SC option loss despite the local visual pass. It is not approved for deployment. The follow-up makes each bounded printed option an inline-block unit and tests its height at the exact Letter printable width. This follows the atomic-box approach described in the [CSS fragmentation specification](https://www.w3.org/TR/css-break-3/), with an explicit fixture height limit to avoid oversized units. CI also exposed a harmless hard-hyphen line wrap (`multi-` followed by `stakeholder` on the next line); PDF text comparison now joins only that line-break pattern, preserving hyphens and ordinary spaces. The PDF gate now runs immediately after generation, without removing any later release checks. The new candidate still requires actual CI-artifact raster inspection before publication.
